...
export default function LitePage() {
  ...
  const handleAddProduct = useCallback((product: Omit<BoxType, 'id' | 'quantity' | 'color'>) => {
    // Add to store by using the store methods
    addBoxType();
    // Then update the last added box with the real data
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over?.id === 'pallet-workspace') {
      // Product was dropped on pallet workspace — trigger generate if needed
      // The product is already in the boxTypes list, just ensure quantity is set
    }
  };

  const handleGenerate = useCallback(() => {
    // Sync quantities to store before generating
    ...
    >
      <div className="h-screen flex flex-col overflow-hidden">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {/* Center: Pallet Workspace or Container 3D */}
